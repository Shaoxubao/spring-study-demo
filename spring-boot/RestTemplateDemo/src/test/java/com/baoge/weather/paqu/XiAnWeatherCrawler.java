...
//        weatherUrlMap.put("国网西咸供电公司", "https://www.lishidata.com/area/%E9%99%95%E8%A5%BF/%E8%A5%BF%E5%92%B8.html");
        ...
        try (Connection conn = getConnection()) {
            for (Map.Entry<String, String> entry : weatherUrlMap.entrySet()) {
                ...
                System.out.println("开始爬取: " + cityName);
                ...
                int successCount = 0;
                int failCount = 0;
                System.out.println("表格总行数: " + rows.size());
                
                for (int i = 1; i < rows.size(); i++) {
                    Element row = rows.get(i);
                    Elements tds = row.select("td");

                    System.out.println("第" + i + "行，列数: " + tds.size());
                    
                    if (tds.size() < 10) {
                        System.out.println("跳过：列数不足");
                        failCount++;
                        continue;
                    }
                    ...
                    System.out.println("原始数据 - 时间:" + timeStr + " 天气:" + weather + " 温度:" + temp + 
                            " 降水:" + rain + " 风向:" + windDir + " 风速:" + windSpeed + " 风力:" + windPower + 
                            " 湿度:" + humidity + " 辐射:" + shortwaveRadiation);

                    LocalDateTime time = parseTime(timeStr);
                    if (time == null) {
                        System.out.println("跳过：时间解析失败 - " + timeStr);
                        failCount++;
                        continue;
                    }
                    System.out.println("解析后时间: " + time);
                    ...
                    System.out.println("解析后数据 - 温度:" + tempValue + " 湿度:" + humidityValue + " 天气编码:" + weatherCode + 
                            " 是否降雨:" + isRain + " 风力等级:" + windLevel + " 风向:" + windDirection + " 风速:" + windSpeedValue +
                            " 辐射:" + shortwaveRadiationValue);
                ...
                System.out.println(cityName + " 成功插入 " + successCount + " 条数据，失败 " + failCount + " 条");
            }
            System.out.println("所有数据爬取完成");
        } catch (SQLException e) {
